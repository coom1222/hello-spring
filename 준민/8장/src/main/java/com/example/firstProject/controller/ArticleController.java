...
public class ArticleController {
    ...
    public String update(ArticleForm form){     // 매개변수로 DTO, 클라이언트에서 수정된 데이터를 DTO에 받아 담기
        // 1. DTO를 엔티티로 변환하기
        Article articleEntity = form.toEntity();    // DTO(form)를 엔티티로 변환하기
        log.info(articleEntity.toString());         // 엔티티로 잘 변환됐는지 로그 찍기
        // 2-1. 엔티티를 DB에 저장하기
        Article target = articleRepository.findById(articleEntity.getId()).orElse(null);  // DB에서 수정할 데이터 가져오기. target데이터에 저장
        // 2-2. 기존 데이터 값을 갱신하기
        if(target != null){
    ...
    public String delete(@PathVariable Long id, RedirectAttributes rttr){
        log.info("삭제할 요청이 들어왔습니다!");
        // 1. 삭제할 대상 가져오기
        Article target = articleRepository.findById(id).orElse(null);
        log.info("타겟에 있는 데이터 : " + target.toString());
        // 2. 대상 엔티티 삭제하기
        if(target != null){
        ...
        return "redirect:/articles";  // 삭제 후 목록 페이지로 이동, articles는 index.mustache 리턴
    }
